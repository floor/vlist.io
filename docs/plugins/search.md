...
- `search()` cannot be combined with `tree()` — creating a list with both throws. Filtering a tree means keeping the ancestors of each match, which belongs to whichever plugin owns the layout; `tree()` provides no such hook, so filtering used to fall through to flat indices and corrupt the list.
