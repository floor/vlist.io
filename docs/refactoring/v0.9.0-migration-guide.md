...
1. **Dimension-Agnostic API**: `HeightCache` → `SizeCache`, `scrollTop` → `scrollPosition`, etc.
2. **Orientation Rename**: `direction` → `orientation`
...
### 1. Config Property: `direction` → `orientation`
...
const list = vlist({
  container: '#list',
  orientation: 'horizontal',  // ✨ Renamed
  item: { width: 200, template: render }
}).build();
```
...
echo "🔄 Migrating to VList v0.9.0..."

# 1. Orientation rename
echo "  → Updating 'direction' to 'orientation'..."
...
# 2. HeightCache → SizeCache
echo "  → Updating HeightCache to SizeCache..."
find . -type f \( -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" \) \
...
# 3. ViewportState properties
echo "  → Updating ViewportState properties..."
find . -type f \( -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" \) \
...
# 4. Event payloads
echo "  → Updating event payloads..."
find . -type f \( -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" \) \
  -exec sed -i.bak 's/{ scrollTop,/{ scrollPosition,/g' {} \; \
  -exec sed -i.bak 's/({ scrollTop/({ scrollPosition/g' {} \;

# 5. Utility functions
echo "  → Updating utility functions..."
find . -type f \( -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" \) \
...
echo "  → Updating BuilderContext methods..."
find . -type f \( -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" \) \
...
echo ""
echo "⚠️  Important: Review the changes and test your application."
...
- **Plugin names**: `withGrid()`, `withSections()`, etc. (unchanged)
...
Sections plugin now works with horizontal orientation:
...
const state: ViewportState = {
  scrollTop: 0,           // ❌ Error: Property 'scrollTop' does not exist
  containerHeight: 600    // ❌ Error: Property 'containerHeight' does not exist
};

// ✅ Correct
const state: ViewportState = {
  scrollPosition: 0,      // ✅ Correct
  containerSize: 600      // ✅ Correct
};
```
...
- [ ] **Custom plugins** - Update any plugin code that uses old API
...
### Issue: Grid plugin error in horizontal mode

**Solution**: Grid plugin doesn't support horizontal orientation (by design).
```typescript
...
```typescript
sizeCache.getSize(index)      // Returns size (height OR width) ✅
state.scrollPosition          // Scroll position (top OR left) ✅
state.containerSize           // Container size (any dimension) ✅
```
