...
Essentials (9)
  ...
  messaging      → "Messaging"          [reverse] [sections]
...
Enhance `messaging`. Add: Headers on/off toggle, Sticky ↔ Inline toggle (`sticky: true/false`), header style selector (date/sender/custom). Keep existing features (auto-scroll, incoming messages, send input, DOM measurement). Footer right side: `reverse · sticky headers`.
