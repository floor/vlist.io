...
| `scroll.mode` | `"native" \| "bounded" \| "synthetic"` | `"native"` | Scroll model; `"synthetic"` needs the `vlist/synthetic` entry. See [Scroll modes](/docs/v2/scroll-modes) |
...
See the [Plugin Authoring tutorial](/tutorials/v2/plugin-authoring) for the full `PluginContext` interface and working examples, or [Exports](./exports.md) for all `vlist/internals` utilities.
