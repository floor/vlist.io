...
| **vlist.ts** | 10 | Core VList class, integrates all plugins |
| **builder/context.ts** | 8 | Builder context management |
| **builder/types.ts** | 6 | Builder type definitions |
| **features/grid/plugin.ts** | 6 | Grid layout plugin |
| **features/grid/renderer.ts** | 6 | Grid rendering logic |
| **features/sections/plugin.ts** | 6 | Sections plugin |
| **features/async/plugin.ts** | 5 | Async loading plugin |
...
4. **Feature Layer**: `features/*` plugins
5. **Integration Layer**: `vlist.ts`, `index.ts`

### Plugin Architecture

All feature plugins follow a consistent pattern:
- Import from `builder/types.ts`
- Import from base `types.ts`
- Often import from `rendering/index.ts`
- Self-contained with minimal cross-plugin dependencies
...
✅ **Plugin isolation** - Features don't depend on each other
...
### Features (8 plugin systems, 29 files total)
- async (5 files)
- grid (5 files)
- page (2 files)
- scale (2 files)
...
2. ✅ Keep plugins isolated - current pattern is excellent
