...
| `/benchmarks/ci-results` | **Automated CI benchmark results** |
...
The benchmark engine can also run under Puppeteer for repeatable local and CI smoke checks. The headless runner reuses the same suite registry and measurement code as the interactive pages: it starts a local vlist.io server, opens Chrome, calls `globalThis.__vlistBenchmarks.runBenchmarks()`, and writes JSON/Markdown artifacts.
...
Run a narrow local smoke test without rebuilding:
...
Generated artifacts are written to `benchmarks/results/` and ignored by git:

| File | Purpose |
|------|---------|
| `latest.json` | Machine-readable result from the most recent run |
| `<timestamp>.json` | Historical local copy of each run |
| `summary.md` | Human-readable metrics table |
| `comparison.md` | Delta report against an optional baseline |

The initial baseline lives at `benchmarks/baselines/main.json`. It was generated from the 10K PR smoke set (`render-vanilla`, `scrollto-vanilla`) and should be refreshed intentionally from a known-good branch, not opportunistically after every run.

CI results can also be stored in SQLite with `bun run bench:store`. Automated runs use separate `ci_benchmark_runs` and `ci_benchmark_metrics` tables so they do not distort crowdsourced public history. Stored CI runs can be inspected on `/benchmarks/ci-results`.

The GitHub Actions workflow (`.github/workflows/perf.yml`) is intentionally non-blocking while benchmark variance is being observed. It runs manually or on pull requests that touch benchmark-relevant paths (`benchmarks/**`, `src/**`, `package.json`, or `bun.lock`). On pull requests it posts a sticky performance comment generated by `bun run bench:comment`, then uploads artifacts for review instead of failing merges on the first regression signal. The workflow checks out `floor/vlist.io` and checks out `floor/vlist` into `../vlist`, matching the local sibling-repository setup. Manual runs can override the `vlist` repository/ref, so `floor/vlist` PRs can later request a benchmark without moving the benchmark harness into the library repo. Once noise is understood, selected budgets in `benchmarks/ci/config.json` can be promoted to hard gates.

For the detailed contributor workflow, see [Automated Performance Workflow](../benchmarks/automated-performance-workflow.md).
...
├── baselines/
│   └── main.json              # Checked-in baseline for headless comparisons
├── ci/
│   ├── runner.mjs             # Puppeteer benchmark runner
│   ├── compare.mjs            # Baseline comparison and Markdown report
│   ├── store.mjs              # SQLite storage for CI-owned results
│   ├── config.json            # Default suites, item counts, and budgets
│   └── README.md              # CI runner command reference
├── results/                   # Generated local/CI artifacts (gitignored)
├── suites/
│   ├── render/
