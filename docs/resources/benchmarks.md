...
The GitHub Actions workflow (`.github/workflows/perf.yml`) is intentionally non-blocking while benchmark variance is being observed. On pull requests it posts a sticky performance comment generated by `bun run bench:comment`, then uploads artifacts for review instead of failing merges on the first regression signal. Once noise is understood, selected budgets in `benchmarks/ci/config.json` can be promoted to hard gates.
