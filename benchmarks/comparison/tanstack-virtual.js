...
const benchmarkTanStackVirtual = async (container, items, onStatus) => {
  onStatus("Testing TanStack Virtual - preparing...");
  ...
  onStatus("Testing TanStack Virtual - rendering...");
  ...
  // Find viewport and measure scroll performance
  onStatus("Testing TanStack Virtual - scrolling...");
  const viewport = findViewport(container);
  const scrollMetrics = await measureScrollPerformance(
    viewport,
    SCROLL_DURATION_MS,
...
defineSuite({
  ...
  run: async ({ itemCount, container, onStatus }) => {
    ...
    if (vlistResults.renderTime && tanstackResults.renderTime) {
      const diff = vlistResults.renderTime - tanstackResults.renderTime;
      const pct = round((diff / tanstackResults.renderTime) * 100, 1);
      ...
      metrics.push({
        label: "TanStack Virtual Render Time",
        value: tanstackResults.renderTime,
        unit: "ms",
        better: "lower",
        rating: rateLower(tanstackResults.renderTime, 30, 50),
      });
      ...
      metrics.push({
        ...
        meta: pct < 0 ? "vlist is faster" : "TanStack Virtual is faster",
      });
    }

    // Memory Comparison
    if (vlistResults.memoryUsed && tanstackResults.memoryUsed) {
      const diff = vlistResults.memoryUsed - tanstackResults.memoryUsed;
      const pct = round((diff / tanstackResults.memoryUsed) * 100, 1);
      ...
      metrics.push({
        label: "TanStack Virtual Memory Usage",
        value: tanstackResults.memoryUsed,
        unit: "MB",
        better: "lower",
        rating: rateLower(tanstackResults.memoryUsed, 30, 50),
      });
      ...
      metrics.push({
        ...
        better: "lower",
        rating: pct < 0 ? "good" : pct < 20 ? "ok" : "bad",
        meta: pct < 0 ? "vlist uses less" : "TanStack Virtual uses less",
      });
    }

    // Scroll FPS Comparison
    if (vlistResults.scrollFPS && tanstackResults.scrollFPS) {
      const diff = vlistResults.scrollFPS - tanstackResults.scrollFPS;
      const pct = round((diff / tanstackResults.scrollFPS) * 100, 1);
      ...
      metrics.push({
        label: "TanStack Virtual Scroll FPS",
        value: tanstackResults.scrollFPS,
        unit: "fps",
        better: "higher",
        rating: rateHigher(tanstackResults.scrollFPS, 55, 50),
      });
      ...
      metrics.push({
        ...
        meta: pct > 0 ? "vlist is smoother" : "TanStack Virtual is smoother",
      });
    }

    // P95 Frame Time Comparison
    if (vlistResults.p95FrameTime && tanstackResults.p95FrameTime) {
      metrics.push({
      ...
      metrics.push({
        label: "TanStack Virtual P95 Frame Time",
        value: tanstackResults.p95FrameTime,
        unit: "ms",
        better: "lower",
        rating: rateLower(tanstackResults.p95FrameTime, 20, 30),
      });
    }

    // Add error metric if TanStack Virtual failed
    if (tanstackResults.error) {
      metrics.push({
        label: "TanStack Virtual Error",
        value: 0,
        unit: "",
        better: "lower",
        rating: "bad",
        meta: tanstackResults.error,
      });
    }
