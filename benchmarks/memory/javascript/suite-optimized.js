// benchmarks/memory/javascript/suite-optimized.js — Memory Benchmark (Optimized)
//
// Same as suite.js but with memory optimizations enabled:
// - copyOnInit: false (no array copy)
// - enableItemById: false (no id→index Map)
//
...
defineSuite({
  id: "memory-javascript-optimized",
  name: "Memory (JavaScript - Optimized)",
  description:
    "Heap usage with memory optimizations enabled (copyOnInit: false, enableItemById: false) — reveals impact of optimizations",
  icon: "🧠",
  ...
  run: async ({ itemCount, container, onStatus }) => {
    ...
    // ── Step 2: Create vlist with optimizations and measure ──────────────
    onStatus("Creating list (optimized)...");
    ...
    const list = vlist({
      ...
      copyOnInit: false, // Don't copy items array
      enableItemById: false, // Don't build id→index Map
    }).build();
    ...
    // Thresholds for optimized version (more aggressive)
    const scrollLeakGood = itemCount <= 100_000 ? 1 : 3;
    const scrollLeakOk = itemCount <= 100_000 ? 5 : 10;

    // Render heap thresholds for optimized version (MB)
    const renderGood = itemCount <= 10_000 ? 2 : itemCount <= 100_000 ? 5 : 30;
    const renderOk = itemCount <= 10_000 ? 5 : itemCount <= 100_000 ? 15 : 80;
