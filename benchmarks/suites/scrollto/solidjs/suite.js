// benchmarks/suites/scrollto/solidjs/suite.js — scrollToIndex Benchmark (SolidJS)
//
...
if (__BENCH_HAS_SYNTHETIC__) defineSuite({
  ...
  run: async ({ itemCount, container, onStatus }) => {
    ...
    try {
      ...
      if (!viewport) throw new Error("Could not find vlist viewport element");
      const result = await measureScrollToPerformance({
        viewport,
        scrollToFn: (index, align) => instance.scrollToIndex(index, align),
