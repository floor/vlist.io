// benchmarks/suites/scroll/svelte/suite.js — Scroll FPS Benchmark (Svelte)
//
// Thin wrapper around the shared engine scroll measurement.
// Defines the vlist-svelte create/destroy lifecycle, calls engine functions,
...
defineSuite({
  ...
  run: async ({
    ...
    onStatus("Warming up...");
    {
      ...
      const warmupAction = vlist(container, {
        config: {
      ...
      if (warmupAction && warmupAction.destroy) {
        warmupAction.destroy();
      }
      container.innerHTML = "";
      await tryGC();
    }
