...
const intParam = (url: URL, name: string, defaultVal: number, min: number, max: number): number => {
    ...
    endpoints: {
      'GET /api/users': {
        ...
        params: {
          ...
          total: { type: 'number', default: DEFAULT_TOTAL, min: 1, max: 10_000_000 }
        },
        response: '{ items: User[], total: number, hasMore: boolean }'
      },
      'GET /api/users/:id': {
...
export const routeApi = async (req: Request): Promise<Response | null> => {
  ...
  // GET /api/info
  if (path === '/api' || path === '/api/' || path === '/api/info') {
    return handleInfo()
  }
