  ...
  setup(build) {
    ...
    build.onLoad({ filter: /^vlist-example-entry$/, namespace: "vlist-example" }, () => ({
      contents: [
        ...
        "  if (globalThis.__VLIST_SCROLLBAR_OWNED) return withSnap;",
        "  if (mode === \"synthetic\") {",
        ...
        "  let current = list;",
        "  const listeners = [];",
        "  const api = new Proxy(list, {",
        "    get(_target, prop) {",
        "      if (prop === \"destroy\") {",
        "        return () => {",
        "          if (activeList === api) activeList = null;",
        "          current.destroy();",
        "        };",
        "      }",
        "      if (prop === \"on\") {",
        ...
        "          return current.on(event, handler);",
        "        };",
        "      }",
        "      if (prop === \"off\") {",
        "        return (event, handler) => {",
        ...
        "    current = next;",
        "    activeList = api;",
        "    for (const entry of listeners) current.on(entry.event, entry.handler);",
        "  };",
        "  return api;",
        "}",
        ...
        "  // An example that rebuilds itself (photo album) already makes fresh",
        "  // plugins and keeps its snapshot. Reusing the stored plugins would",
        "  // restore the position and then let the old grid tear the new list down.",
        "  if (typeof globalThis.__vlistRecreate === \"function\") {",
        "    await globalThis.__vlistRecreate();",
        "    return true;",
        "  }",
        "  if (!activeList || !activeRetarget) return false;",
        "  // The list is rebuilt from the config it was created with. An example",
        "  // that loads its data afterwards (social feed: fetch, then setItems)",
        "  // was created with no items, so the switch came back empty. The live",
        "  // items win over the config's; an adapter-driven list has none and",
        "  // keeps its adapter.",
        "  const accessor = activeList.items;",
        "  const live = Array.isArray(accessor) ? accessor : accessor && typeof accessor.all === \"function\" ? accessor.all() : null;",
        "  const config = live && live.length > 0 ? { ...activeConfig, items: live.slice() } : activeConfig;",
        "  const plugins = activePlugins;",
        "  const retarget = activeRetarget;",
        ...
        "  if (typeof next.refreshScrollbar === \"function\") next.refreshScrollbar();",
        "  return true;",
        "};",
      ].join("\n"),
      loader: "js",
